...
Cinsiyet belirlemek için sadece: \`${p}erkek /${p}kadın\` yazman yeterli. \<a:andran_ok:862048102831358002>
`)
//.setColor("#00d5ff")
message.react('862048102831358002')
...
kullanici.roles.remove(ayarlar.unregister2)
  

const onayembed = new MessageEmbed()
//.setColor("RANDOM")
.setAuthor(message.guild.name, message.guild.iconURL({ dynamic: true }))
.setDescription(`${kullanici} kişisi başarıyla erkek olarak kaydedildi.`)
...
//client.channels.cache.get(ayarlar.genelChat).send(new MessageEmbed().setAuthor('Calavera Welcome').setDescription(`**${kullanici} Adlı Kullanıcı Aramıza katıldı ! hoşgeldin umarım keyifli vakit geçirirsin.**`).setColor('BLUE'))
...
kullanici.roles.add(ayarlar.kadın3)
kullanici.roles.remove(ayarlar.unregister)
kullanici.roles.remove(ayarlar.unregister2)
